...
export const SCHEMA_TEMPLATES = [
  ...
  // ── 4. Social Network ─────────────────────────────────────────────────────
  {
    ...
    nodes: [
      { id:'sn_users',         type:'tableNode', position:{x:80,  y:80},  data:{name:'users',         columns:[pkCol('snu1'),col('snu2','name','VARCHAR'),col('snu3','email','VARCHAR',{unique:true}),col('snu4','password','VARCHAR'),col('snu5','bio','TEXT',{nullable:true}),col('snu6','avatar_url','VARCHAR',{nullable:true})]}},
      { id:'sn_posts',         type:'tableNode', position:{x:460, y:80},  data:{name:'posts',         columns:[pkCol('snp1'),fkCol('snp2','user_id'),col('snp3','body','TEXT'),col('snp4','image_url','VARCHAR',{nullable:true}),col('snp5','created_at','DATETIME')]}},
  ...
  // ── 5. Job Board ──────────────────────────────────────────────────────────
  {
    ...
    nodes: [
      { id:'jb_companies',   type:'tableNode', position:{x:80,  y:80},  data:{name:'companies',   columns:[pkCol('jbc1'),col('jbc2','name','VARCHAR'),col('jbc3','website','VARCHAR',{nullable:true}),col('jbc4','description','TEXT',{nullable:true})]}},
      ...
      { id:'jb_applications',type:'tableNode', position:{x:840, y:80},  data:{name:'applications',columns:[pkCol('jba1'),fkCol('jba2','job_id'),fkCol('jba3','candidate_id'),col('jba4','status','ENUM',{default:'applied'}),col('jba5','cover_letter','TEXT',{nullable:true}),col('jba6','applied_at','DATETIME')]}},
  ...
  // ── 6. Booking & Reservation ──────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'bk_bookings', type:'tableNode', position:{x:460, y:360}, data:{name:'bookings', columns:[pkCol('bkb1'),fkCol('bkb2','user_id'),fkCol('bkb3','service_id'),fkCol('bkb4','provider_id'),col('bkb5','scheduled_at','DATETIME'),col('bkb6','status','ENUM',{default:'pending'})]}},
      ...
      { id:'bk_reviews',  type:'tableNode', position:{x:840, y:360}, data:{name:'reviews',  columns:[pkCol('bkr1'),fkCol('bkr2','booking_id'),fkCol('bkr3','user_id'),col('bkr4','rating','INT'),col('bkr5','comment','TEXT',{nullable:true})]}},
    ],
    edges: [
  ...
  // ── 7. Hospital & Clinic ──────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'hc_doctors',        type:'tableNode', position:{x:460, y:80},  data:{name:'doctors',        columns:[pkCol('hcdr1'),fkCol('hcdr2','department_id'),col('hcdr3','name','VARCHAR'),col('hcdr4','email','VARCHAR',{unique:true}),col('hcdr5','phone','VARCHAR'),col('hcdr6','specialization','VARCHAR')]}},
      { id:'hc_patients',       type:'tableNode', position:{x:840, y:80},  data:{name:'patients',       columns:[pkCol('hcp1'),col('hcp2','name','VARCHAR'),col('hcp3','email','VARCHAR',{unique:true}),col('hcp4','phone','VARCHAR'),col('hcp5','date_of_birth','DATE'),col('hcp6','blood_type','VARCHAR',{nullable:true})]}},
  ...
  // ── 8. School & University ────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'su_teachers',    type:'tableNode', position:{x:460, y:80},  data:{name:'teachers',    columns:[pkCol('sut1'),fkCol('sut2','department_id'),col('sut3','name','VARCHAR'),col('sut4','email','VARCHAR',{unique:true}),col('sut5','phone','VARCHAR',{nullable:true})]}},
      { id:'su_courses',     type:'tableNode', position:{x:460, y:360}, data:{name:'courses',     columns:[pkCol('suc1'),fkCol('suc2','department_id'),fkCol('suc3','teacher_id'),fkNullCol('suc4','classroom_id'),col('suc5','name','VARCHAR'),col('suc6','code','VARCHAR',{unique:true}),col('suc7','credits','INT')]}},
      { id:'su_students',    type:'tableNode', position:{x:80,  y:360}, data:{name:'students',    columns:[pkCol('sust1'),fkCol('sust2','department_id'),col('sust3','name','VARCHAR'),col('sust4','email','VARCHAR',{unique:true}),col('sust5','enrolled_on','DATE')]}},
  ...
  // ── 9. Library Management ─────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'lb_books',        type:'tableNode', position:{x:460, y:80},  data:{name:'books',        columns:[pkCol('lbb1'),fkCol('lbb2','category_id'),col('lbb3','title','VARCHAR'),col('lbb4','isbn','VARCHAR',{unique:true}),col('lbb5','pub_year','INT'),col('lbb6','total_copies','INT'),col('lbb7','avail_copies','INT')]}},
      ...
      { id:'lb_members',      type:'tableNode', position:{x:80,  y:360}, data:{name:'members',      columns:[pkCol('lbm1'),col('lbm2','name','VARCHAR'),col('lbm3','email','VARCHAR',{unique:true}),col('lbm4','phone','VARCHAR'),col('lbm5','expires_at','DATE')]}},
      { id:'lb_loans',        type:'tableNode', position:{x:80,  y:640}, data:{name:'loans',        columns:[pkCol('lbl1'),fkCol('lbl2','book_id'),fkCol('lbl3','member_id'),col('lbl4','loaned_at','DATE'),col('lbl5','due_at','DATE'),col('lbl6','returned_at','DATE',{nullable:true})]}},
  ...
  // ── 10. Real Estate ───────────────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'re_agents',          type:'tableNode', position:{x:460, y:80},  data:{name:'agents',          columns:[pkCol('reag1'),fkCol('reag2','agency_id'),col('reag3','name','VARCHAR'),col('reag4','email','VARCHAR',{unique:true}),col('reag5','phone','VARCHAR'),col('reag6','license_no','VARCHAR')]}},
      { id:'re_properties',      type:'tableNode', position:{x:460, y:360}, data:{name:'properties',      columns:[pkCol('rep1'),fkCol('rep2','agent_id'),fkCol('rep3','type_id'),col('rep4','address','VARCHAR'),col('rep5','city','VARCHAR'),col('rep6','area_sqft','DECIMAL'),col('rep7','bedrooms','INT',{nullable:true}),col('rep8','bathrooms','INT',{nullable:true})]}},
      ...
      { id:'re_offers',          type:'tableNode', position:{x:80,  y:640}, data:{name:'offers',          columns:[pkCol('reo1'),fkCol('reo2','listing_id'),col('reo3','buyer_name','VARCHAR'),col('reo4','offer_price','DECIMAL'),col('reo5','status','ENUM',{default:'pending'}),col('reo6','offered_at','DATE')]}},
  ...
  // ── 11. Restaurant & Food Delivery ────────────────────────────────────────
  {
    ...
    nodes: [
      { id:'fd_restaurants', type:'tableNode', position:{x:80,  y:80},  data:{name:'restaurants', columns:[pkCol('fdr1'),col('fdr2','name','VARCHAR'),col('fdr3','address','VARCHAR'),col('fdr4','phone','VARCHAR'),col('fdr5','rating','DECIMAL',{nullable:true})]}},
      ...
      { id:'fd_orders',      type:'tableNode', position:{x:460, y:360}, data:{name:'orders',      columns:[pkCol('fdo1'),fkCol('fdo2','customer_id'),fkCol('fdo3','restaurant_id'),col('fdo4','status','ENUM',{default:'pending'}),col('fdo5','total_amount','DECIMAL'),col('fdo6','placed_at','DATETIME')]}},
      ...
      { id:'fd_drivers',     type:'tableNode', position:{x:80,  y:640}, data:{name:'drivers',     columns:[pkCol('fddv1'),col('fddv2','name','VARCHAR'),col('fddv3','phone','VARCHAR',{unique:true}),col('fddv4','vehicle_plate','VARCHAR'),col('fddv5','is_available','BOOLEAN',{default:'1'})]}},
  ...
  // ── 12. Authentication & Roles ────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'ar_roles',           type:'tableNode', position:{x:460, y:80},  data:{name:'roles',           columns:[pkCol('arr1'),col('arr2','name','VARCHAR',{unique:true}),col('arr3','description','TEXT',{nullable:true})]}},
      { id:'ar_permissions',     type:'tableNode', position:{x:840, y:80},  data:{name:'permissions',     columns:[pkCol('arp1'),col('arp2','name','VARCHAR',{unique:true}),col('arp3','description','TEXT',{nullable:true})]}},
  ...
  // ── 13. CMS ───────────────────────────────────────────────────────────────
  {
    ...
    nodes: [
      { id:'cm_users',     type:'tableNode', position:{x:80,  y:80},  data:{name:'users',     columns:[pkCol('cmu1'),col('cmu2','name','VARCHAR'),col('cmu3','email','VARCHAR',{unique:true}),col('cmu4','password','VARCHAR'),col('cmu5','role','ENUM',{default:'author'})]}},
      ...
      { id:'cm_media',     type:'tableNode', position:{x:80,  y:360}, data:{name:'media',     columns:[pkCol('cmmed1'),fkCol('cmmed2','user_id'),col('cmmed3','filename','VARCHAR'),col('cmmed4','path','VARCHAR'),col('cmmed5','mime_type','VARCHAR'),col('cmmed6','size_bytes','INT')]}},
      { id:'cm_pages',     type:'tableNode', position:{x:460, y:360}, data:{name:'pages',     columns:[pkCol('cmpg1'),fkCol('cmpg2','user_id'),col('cmpg3','title','VARCHAR'),col('cmpg4','slug','VARCHAR',{unique:true}),col('cmpg5','content','LONGTEXT'),col('cmpg6','status','ENUM',{default:'draft'})]}},
  ...
  // ── 14. Inventory Management ──────────────────────────────────────────────
  {
    ...
    nodes: [
      ...
      { id:'iv_suppliers',   type:'tableNode', position:{x:460, y:80},  data:{name:'suppliers',           columns:[pkCol('ivs1'),col('ivs2','name','VARCHAR'),col('ivs3','email','VARCHAR',{unique:true}),col('ivs4','phone','VARCHAR'),col('ivs5','address','TEXT',{nullable:true})]}},
      { id:'iv_warehouses',  type:'tableNode', position:{x:840, y:80},  data:{name:'warehouses',          columns:[pkCol('ivw1'),col('ivw2','name','VARCHAR'),col('ivw3','location','VARCHAR'),col('ivw4','capacity','INT')]}},
      { id:'iv_products',    type:'tableNode', position:{x:80,  y:360}, data:{name:'products',            columns:[pkCol('ivp1'),fkCol('ivp2','category_id'),col('ivp3','name','VARCHAR'),col('ivp4','sku','VARCHAR',{unique:true}),col('ivp5','unit','VARCHAR'),col('ivp6','reorder_level','INT')]}},
  ...
  // ── 15. Multi-tenant SaaS ─────────────────────────────────────────────────
  {
    ...
    nodes: [
      { id:'mt_tenants',         type:'tableNode', position:{x:80,  y:80},  data:{name:'tenants',         columns:[pkCol('mtt1'),col('mtt2','name','VARCHAR'),col('mtt3','domain','VARCHAR',{unique:true}),col('mtt4','status','ENUM',{default:'active'})]}},
      ...
      { id:'mt_users',           type:'tableNode', position:{x:80,  y:360}, data:{name:'users',           columns:[pkCol('mtu1'),fkCol('mtu2','tenant_id'),col('mtu3','name','VARCHAR'),col('mtu4','email','VARCHAR',{unique:true}),col('mtu5','password','VARCHAR'),col('mtu6','role','ENUM',{default:'member'}),col('mtu7','is_active','BOOLEAN',{default:'1'})]}},
